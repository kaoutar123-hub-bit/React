...
export function usePosts() {
    ...
    const del = async (id) => {
        ...
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/publicaciones/${id}`, {
                method: "DELETE",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    "Content-Type": "application/json",
    ...
    const update = async (postId, formData/*contenido*/) => {
        ...
        // Llamada al BackEnd (Laravel)
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL}/api/publicaciones/${postId}`, {
                method: "POST",
                headers: {
                    "Authorization": `Bearer ${token}`,
                    //"Content-Type": "application/json",
